...
class BusRoutes {
  ...
  public CompletableFuture<String> description() {
    ...
    CompletableFuture<String> fullResult = CompletableFuture.completedFuture(result);
    for (BusService service : services.keySet()) {
      CompletableFuture<Set<BusStop>> stops = services.get(service);
      fullResult = fullResult.thenCombineAsync(describeService(service, stops), (a, b) -> a + b);
    }
    return fullResult;
  }
